...
def download_model_from_kaggle(kernel_name, output_files, dest_folder):
    try:
        ...
        if not os.path.exists(kaggle_json_path):  # Hanya buat file jika belum ada
            with open(kaggle_json_path, 'w') as f:
                json.dump({"username": kaggle_username, "key": kaggle_key}, f)

            st.success("API Key Kaggle berhasil disalin ke ~/.kaggle/kaggle.json")
        else:
            st.warning("API Key Kaggle sudah ada.")
        ...
        st.success(f"Model berhasil diunduh ke {dest_folder}")
...
if os.path.exists(melspec_model_save_path):
    ...
    st.error(f"Model Melspec tidak ditemukan di {melspec_model_save_path}")
...
if os.path.exists(mfcc_model_save_path):
    ...
    st.error(f"Model MFCC tidak ditemukan di {mfcc_model_save_path}")
...
Aplikasi ini mengimplementasikan teknik ekstraksi fitur audio menggunakan MFCC dan Melspectrogram, serta menggunakan dua model CNN yang berbeda untuk mengklasifikasikan suara burung yang ada di Indonesia Bagian Barat. Unggah file audio dalam format MP3 atau WAV, dan model akan memberikan prediksi kelas burung beserta akurasi dari kedua model.
""")
...
if uploaded_audio is not None:
    st.audio(uploaded_audio, format="audio/mp3")  # Menampilkan audio yang diunggah
