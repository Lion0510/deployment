...
if uploaded_audio is not None:
    ...
    if st.button('Prediksi Kelas Burung'):
        with st.spinner("Memproses..."):
            # Download models from Google Drive if they are not already downloaded
            mel_model_path = 'cnn_melspec.h5'
            mfcc_model_path = 'cnn_mfcc.h5'

            if not os.path.exists(mel_model_path):
                gdown.download(melspec_model_url, mel_model_path, quiet=False)
            
            if not os.path.exists(mfcc_model_path):
                gdown.download(mfcc_model_url, mfcc_model_path, quiet=False)

            # Load the pre-trained models after downloading
            melspec_model = load_model(mel_model_path)
